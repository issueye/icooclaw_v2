...
func tryRunBundledExecutable(scriptArgs []string) (bool, error) {
	...
	opts, remainingArgs, err := parseRuntimeOptions(scriptArgs)
	if err != nil {
		return true, err
	}

	if len(payload.Files) == 0 {
		return true, executeScriptSource(payload.ScriptPath, payload.ScriptSource, remainingArgs, opts)
	}
	...
	return true, runFileForBundle(scriptPath, remainingArgs, opts)
}
...
func runFileForBundle(filename string, scriptArgs []string, opts runtimeOptions) error {
