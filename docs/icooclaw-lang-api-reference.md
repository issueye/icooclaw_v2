...
```bash
iclang run [--max-goroutines n] [--max-memory-mb n] [--max-memory-percent n] <file.is> [args...]
...
- 打包后的程序也支持 runtime 参数，例如 `demo.exe --max-goroutines 4 --max-memory-percent 90 input.txt`
...
- `iclang run --max-memory-mb n ...` 会为当前脚本设置内存上限
- `iclang run --max-memory-percent n ...` 会按主机总内存百分比设置上限
- `iclang repl --max-memory-mb n` 会为当前 REPL 设置内存上限
- `iclang repl --max-memory-percent n` 会按主机总内存百分比设置上限
- 打包后的程序同样支持 `--max-memory-mb n`
- 打包后的程序同样支持 `--max-memory-percent n`
- 如果没有显式配置，默认阈值是主机总内存的 `90%`
- 优先级是：CLI `--max-memory-mb` > CLI `--max-memory-percent` > 环境变量 `ICLANG_MAX_MEMORY_MB` > 环境变量 `ICLANG_MAX_MEMORY_PERCENT` > 默认 `90%`
- 当解析或执行阶段超过上限时，解释器会返回 `memory limit exceeded` 错误，而不是继续运行到进程 OOM
